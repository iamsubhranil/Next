  ...
  public:
		...
		if(m_absolute) {
			if(m_type == posix_path)
				...
				for(size_t i = 0; i < m_path.size(); ++i)
					// No special case for the last segment to count the NULL
					// character
					length += m_path[i].length() + 1;
				if(m_smb)
					length += 2;

				// Windows requires a \\?\ prefix to handle paths longer than
				// MAX_PATH (including their null character). NOTE: relative
				// paths >MAX_PATH are not supported at all in Windows.
	...
	void set(const std::string &str, path_type type = native_path) {
		...
		if(type == windows_path) {
			...
			// Long windows paths (sometimes) begin with the prefix \\?\. It
			// should only be used when the path is >MAX_PATH characters long,
			// so we remove it for convenience and add it back (if necessary) in
			// str()/wstr().
			...
			if(tmp.length() >= 2 && tmp[0] == '\\' && tmp[1] == '\\') {
				...
				// Interestingly, there is a special-special case where relative
				// paths may be specified as beginning with a "\\" when a
				// non-SMB file with a more-than-260-characters-long absolute
				// _local_ path is double-clicked. This seems to only happen
				// with single-segment relative paths, so we can check for this
				// condition by making sure no further path separators are
				// present.
				...
				// Special-case handling of absolute SMB paths, which start with
				// the prefix "UNC\"
				...
				m_absolute = true;
				m_smb      = true;
				// Special-case handling of absolute local paths, which start
				// with the drive letter and a colon "X:\"
			} else if(tmp.length() >= 3 && std::isalpha(tmp[0]) &&
			          tmp[1] == ':' && (tmp[2] == '\\' || tmp[2] == '/')) {
...
#else
		HANDLE handle = CreateFileW(wstr().c_str(), GENERIC_WRITE, 0, nullptr,
		                            0, FILE_ATTRIBUTE_NORMAL, nullptr);
	...
	std::wstring wstr(path_type type = native_path) const {
		...
		int size = MultiByteToWideChar(CP_UTF8, 0, &temp[0], (int)temp.size(),
		                               NULL, 0);
	...
	void set(const std::wstring &wstring, path_type type = native_path) {
		...
		if(!wstring.empty()) {
			int size =
			    WideCharToMultiByte(CP_UTF8, 0, &wstring[0],
